...
#include "unity_internals.h"
     ...
     * to support complex test loops, etc. Both are NOT built into Unity.
     * Instead the test runner generator will create them. resetTest will run
     * teardown and setup again, verifying any end-of-test needs between.
     * verifyTest will only run the verification. */
    void resetTest(void);
...
#define TEST_ASSERT_FLOAT_ARRAY_WITHIN_MESSAGE(     \
    delta, expected, actual, num_elements, message) \
    UNITY_TEST_ASSERT_FLOAT_ARRAY_WITHIN(           \
        (delta), (expected), (actual), (num_elements), __LINE__, (message))
...
#define TEST_ASSERT_NOT_EQUAL_DOUBLE_MESSAGE(expected, actual, message) \
    UNITY_TEST_ASSERT_NOT_EQUAL_DOUBLE(                                 \
        (expected), (actual), __LINE__, (message))
#define TEST_ASSERT_DOUBLE_ARRAY_WITHIN_MESSAGE(    \
    delta, expected, actual, num_elements, message) \
    UNITY_TEST_ASSERT_DOUBLE_ARRAY_WITHIN(          \
        (delta), (expected), (actual), (num_elements), __LINE__, (message))
...
#define TEST_ASSERT_GREATER_THAN_DOUBLE_MESSAGE(threshold, actual, message) \
    UNITY_TEST_ASSERT_GREATER_THAN_DOUBLE(                                  \
        (threshold), (actual), __LINE__, (message))
#define TEST_ASSERT_GREATER_OR_EQUAL_DOUBLE_MESSAGE( \
    threshold, actual, message)                      \
    UNITY_TEST_ASSERT_GREATER_OR_EQUAL_DOUBLE(       \
        (threshold), (actual), __LINE__, (message))
